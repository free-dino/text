...
# Simplexes
A family of polyhedra. Suppose the $k+1$ points $v_0, \dots, v_k \in \R^n$ are affinely independent, which means $v_1 - v_0, \dots, v_k-v_0$ are linearly independent. The simplex determined by them is given by 
$$
C = \textbf{conv} \{ v_0, \dots, v_k \} = \{\theta_0 v_0 + \cdots + \theta_k v_k \mid \theta \succeq 0, \v 1^\top \theta = 1\}
$$